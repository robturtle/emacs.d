...
#+BEGIN_SRC emacs-lisp
  (defun yang/get-java-package ()
      ...
      (if (string-match-p "src/main/java/" current-dir)
          (setq package (substring current-dir
                                   (+ (string-match "src/main/java/" current-dir)
                                      (length "src/main/java/"))
                                   -1))
        (if (string-match-p "src/" current-dir)
            (setq package (substring current-dir
                                     (+ (string-match "src/" current-dir)
                                        (length "src/"))
                                     -1))
          (if (string-match-p "test/" current-dir)
              (setq package (substring current-dir
                                       (+ (string-match "test/" current-dir)
                                          (length "test/"))
                                       -1))
            (setq package current-dir))
          )
        )
