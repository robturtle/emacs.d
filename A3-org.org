...
* Trouble-shooting
** FIXME viod function is-habit-p
#+header: :tangle yes
...
#+results: org-mode-version
: release_8.3beta-1075-gca2b398

This is version call_org-mode-doc-version() {{{results(=nil=)}}} of this document.  This
document is created using the publishing features of =org-mode= git
version call_org-mode-version() {{{results(=release_8.3.1-234-g8c85c99=)}}}.
...
| NYU.org  | related to my graduate admission fairs     |
...
#+begin_src emacs-lisp :exports none
  ...
  (if (boundp 'org-user-agenda-files)
    ...
    (progn
      (shell-command "[[ -d \"${HOME}/git/org\" ]] || mkdir -p ${HOME}/git/org")
      (setq org-agenda-files (quote ("~/git/org")))))
#+end_src
...
| C-c C-x C-j | Goto currently clocked item                     | Very Often |
| C-c c       | Capture a task                                  | Very Often |
| C-c C-x TAB | Clock in a task (show menu with prefix)         | Often      |
| C-c o g     | Gnus - I check mail regularly                   | Often      |
| C-c o t     | Show todo items for this subtree                | Often      |
| C-c o W     | Widen                                           | Often      |
| C-c o b     | Quick access to bbdb data                       | Often      |
| C-c o c     | Calendar access                                 | Often      |
| C-c o P     | Save buffers and publish current project        | Often      |
| C-c l       | Store a link for retrieval with C-c C-l         | Often      |
| C-c o y     | Go to next org file in org-agenda-files         | Sometimes  |
| C-c o R     | Boxquote selected region                        | Sometimes  |
| C-c o v     | Toggle visible mode (for showing/editing links) | Sometimes  |
| C-c o ,     | Previous buffer                                 | Sometimes  |
| C-c o .     | Next buffer                                     | Sometimes  |
| C-x n r     | Narrow to region                                | Sometimes  |
| C-c o f     | Boxquote insert a file                          | Sometimes  |
| f9 i        | Info manual                                     | Sometimes  |
| C-c o I     | Punch Clock In                                  | Sometimes  |
| C-c o O     | Punch Clock Out                                 | Sometimes  |
| C-c o S     | Switch to org scratch buffer                    | Sometimes  |
| C-c o s     | Switch to scratch buffer                        | Sometimes  |
| C-c o h     | Hide other tasks                                | Rare       |
| C-c o T     | Toggle insert inactive timestamp                | Rare       |
...
#+begin_src emacs-lisp
;; Custom Key Bindings
(global-set-key (kbd "C-c o t") 'bh/org-todo)
(global-set-key (kbd "C-c o W") 'bh/widen)
(global-set-key (kbd "C-c o y") 'org-cycle-agenda-files)
(global-set-key (kbd "C-c o o") 'bh/show-org-agenda)
(global-set-key (kbd "C-c o b") 'bbdb)
(global-set-key (kbd "C-c o c") 'calendar)
(global-set-key (kbd "C-c o f") 'boxquote-insert-file)
(global-set-key (kbd "C-c o g") 'gnus)
(global-set-key (kbd "C-c o h") 'bh/hide-other)
(global-set-key (kbd "C-c o n") 'bh/toggle-next-task-display)

(global-set-key (kbd "C-c o I") 'bh/punch-in)
(global-set-key (kbd "C-c o O") 'bh/punch-out)

(global-set-key (kbd "C-c o s") 'bh/make-org-scratch)

(global-set-key (kbd "C-c o R") 'boxquote-region)
(global-set-key (kbd "C-c o S") 'bh/switch-to-scratch)

(global-set-key (kbd "C-c o T") 'bh/toggle-insert-inactive-timestamp)

(global-set-key (kbd "C-c o v") 'visible-mode)
(global-set-key (kbd "C-c o l") 'org-toggle-link-display)
(global-set-key (kbd "C-c o SPC") 'bh/clock-in-last-task)
(global-set-key (kbd "C-c o ,") 'previous-buffer)
(global-set-key (kbd "C-c o m") 'org-toggle-inline-images)
(global-set-key (kbd "C-c o r") 'narrow-to-region)
(global-set-key (kbd "C-c o .") 'next-buffer)
(global-set-key (kbd "C-c o P") 'bh/save-then-publish)
...
#+header: :tangle yes
#+begin_src emacs-lisp
  (shell-command "[[ -d \"${HOME}/git/org\" ]] || mkdir ${HOME}/git/org")
  (shell-command "[[ -d \"${HOME}/git/org/refile.org\" ]] || touch ${HOME}/git/org/refile.org")
...
Here is the setting to complete using ido.  And now I've quit ido.
#+header: :tangle no
#+BEGIN_SRC emacs-lisp
...
Now I've switched from ido to helm. 
*** FIXME add use case for helm
** Refiling Tasks
:PROPERTIES:
...
(defun bh/org-auto-exclude-function (tag)
  ...
  (and (cond
        ((string= tag "hold") t)
	((string= tag "waiting") t)
	((string= tag "@network") t)
	((string= tag "@home") t)
	((string= tag "game") t)
       )
       (concat "-" tag)))
...
I'm a student and I always learn things.  So I have a org file
=study.org= comprises of all learning quests.  It's good to let the
clock modeline to just show the clocksum only on today.  To do so, I
set a property at the header of the file:

#+BEGIN_EXAMPLE
#+PROPERTY: CLOCK_MODELINE_TOTAL today
#+END_EXAMPLE

And to let all the sub-projects in the file inherit this property, I
should toggle this option:
...
#+header: :tangle no
#+begin_src emacs-lisp
;; This is an example, do not tangle it
...
(global-set-key (kbd "C-c o p") 'bh/phone-call)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; I also want to see the utf-8 math symbols at start-up
  (setq org-pretty-entities t)
#+END_SRC

The image don't refresh quickly, maybe I need to reset this to fix it:
#+header: :tangle yes
#+BEGIN_SRC emacs-lisp
...
#+begin_src ditaa :file communication.png :cmdline -r -s 0.8 :exports both :cache yes
  ...
  +--------+--------+                                           +-------------+
  |                 |                                           |             |
  | Millwide System |            -------- Data ---------        | My test box |
  | cBLU            |            --=----- Signals ---=--        |             |
  +-----------------+                                           | cPNK        |
                                                                |             |
                                                                |             |
...
#+results[82bf0236e1fb799313e19f0c299e1810a11a75dd]:
...
** Latex exporting settings
I want to create a class for my homeworks so they will automatically
share the same layout and settings.  In order to do this, you must
know how to create a LaTex class file and put it into a tex home
directory so latex program can find it from everywhere.  I create [[https://gist.github.com/robturtle/37bd27d34c204e5f07b7][a
gist]] to demonstrate this process.  And the final step is to add my
class into the variable =org-latex-classes=:

#+header: :tangle yes
#+BEGIN_SRC emacs-lisp
  (add-to-list 'org-latex-classes
               '("networkHW" "\\documentclass{networkHW}"
                 ("\\section{%s}" . "\\section*{%s}")
...
** [done] Abbrev-mode and Skeletons -- Switch to Snippets now
:PROPERTIES:
...
I still like to see the default diary.
...
#+header: :tangle no
#+begin_src emacs-lisp
(global-set-key (kbd "C-c o t") 'bh/insert-inactive-timestamp)
#+end_src
...
server. If you are using a public server, you should consider
encrypting the files that are uploaded to the server. This can be done
with Org mode 7.02 and with MobileOrg 1.5 (iPhone version), and you
need an openssl installation on your system. To turn on encryption,
set a password in MobileOrg and, on the Emacs side, configure the
variable org-mobile-use-encryption1.

The easiest way to create that directory is to use a free Dropbox.com
account². When MobileOrg first connects to your Dropbox, it will
create a directory MobileOrg inside the Dropbox. After the directory
has been created, tell Emacs about it:

#+header: :tangle yes
#+BEGIN_SRC emacs-lisp
  (setq org-mobile-directory "/Users/jeremy/Dropbox/Apps/MobileOrg/")
  (setq org-mobile-inbox-for-pull "/Users/jeremy/git/org/todo.org")
#+END_SRC

After that we will want it automatically sync with the dropbox:
